...
(function ($) {
    ...
    /** Actual plugin code **/
    $.fn.raindrop = function (event, options) {
        ...
        if (typeof event === 'undefined' || event === 'start') {
            ...
            var petalCreator = function () {
                if (target.data('raindrop-anim-id')) {
                    setTimeout(function () {
                ...
                var fallTime = ((document.documentElement.clientHeight * 0.005) + Math.round(1 * 5)) * options.fallSpeed;
                ...
                var animations =
                    'fall ' + fallTime 
                        + 's linear 0s 1' 
                        + ', ' +
                        blowAnimation + ' ' + (((fallTime > 30 ? fallTime : 30) - 20) + 5) + 's linear 0s infinite'
                        // + ', ' +
                ...
                var width = height*0.05;
                //var width  = height - Math.floor(randomInt(0, options.minSize) / 1.5);
                ...
                .css({
                    ...
                    //'border-radius': randomInt(options.maxSize, (options.maxSize + Math.floor(Math.random() * 1))) + 'px ' + randomInt(1, Math.floor(width / 1)) + 'px',
                    height: height + 'px',
                    left: (2*Math.random() * document.documentElement.clientWidth) + 'px',
                    'margin-top': (-(Math.floor(Math.random() * 10) + 30)) + 'px',
                    width: width + 'px'
                });
            ...
            target.data('raindrop-anim-id', requestAnimationFrame(petalCreator));
        ...
        // Stop event, which stops the animation loop and removes all current blossoms
            ...
            // Cancel animation
            var animId = target.data('raindrop-anim-id');

            if (animId) {
                cancelAnimationFrame(animId);
                target.data('raindrop-anim-id', null);
            }
